...
//	private float v = 8;
	...
	public void setup(){
		...
		// initialize masks
		fMask = createImage(width,height,ARGB);
		mhMask = createImage(width,height,ARGB);
	...
	public void draw(){
		...
		if (canvasArm.isDetected &&  canvasArm.axisB.z < far && canvasArm.axisA.z < far)
...
//				c.setPolygonApproximationFactor(3);
	...
	private void detectCanvasArm(Contour c) {
		 ...
		 */
		if (canvasCalibration && canvasArm.isDetected && canvasArm.bb.x > 0){
	...
	private class Forearm{
		...
		public void findTouch(){
			...
			if (modelingHand.isDetected && nearTouch && modelingHand.fingers[3] != null){
				PVector p0 = modelingHand.selPos;
...
//						println();
	...
	private class Hand {
		...
		private PVector pinchPos = new PVector(); // SHOULD JUST USE selPos, I think
